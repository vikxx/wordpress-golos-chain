...
//│ │ ┌────────── Установить: npm install --save wpapi
...
// Укажите в postLimit, сколько последних постов проверять на wordpress блоге. 
// Разумно комбинируйте это со значением globalInterval - не указывайте слишком большее количество постов при слишком коротком интервале
// Если блог-донор обновляется редко - указывайте неблольшой значение.
const postLimit = 4

// ИНТЕРВАЛ РАЗМЕЩЕНИЯ ПОСТОВ
// Используйте разумно из расчета расписания указанного вами в CRON, а также помните, что минимальный интервал публикации в блокчейн 3 секунды
// 10000 = 10 секунд. Посты из каждой полученной серии постов (серия будет состоять из количества в postLimit) 
// Будут публиковаться по 1 в 10 секунд, если вы не зададите больший интевал. 
...
// Ниже создадим функцию, которая будет транслитеровать кириллицу в латиницу.
// Напротив каждой кириллической буквы мы поместим ее латиницкий аналог в формате голоса:	
const cyrTag = () => {
	// Таблицв транслитерации в том виде, в котором она принята на golos.io
  const _associations = {
    "а": "a",
    "б": "b",
    "в": "v",
...
// Теперь мы сможем транслитеровать теги подобным образом: cyrTag().transform('Тег на русском', "-")) 
