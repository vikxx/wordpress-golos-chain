//┌────────────── ИМПОРТ И СИНХРОНИЗАЦИЯ WORDPRESS БЛОГОВ С БЛОГАМИ НА GOLOS И STEEM
...
// НАСТРОЙКИ
// Ниже необходимо указать путь до wp-json директивы вашего WORDPRESS 
const wp = new WP({
...
// Если вы ошиблись при размещении поста - вы можете оперативно отредактировать его в WP с другим форматом -
// Это изменит формат и на голосе. 
...
// Разумно комбинируйте это со значением postInterval - не указывайте слишком большoе количество постов при слишком коротком интервале
// Если блог-донор обновляется редко - указывайте небольшое значение.
const postLimit = 1

// ИНТЕРВАЛ РАЗМЕЩЕНИЯ ПОСТОВ
// Используйте разумно из расчета расписания, указанного вами в CRON, а также помните, что минимальный интервал
// должен быть минимум 5 минут - голос не разрешает размещать посты чаще. 
...
// ws://localhost:9090 - порт 9090 у вас может быть другим, он указан в конфиге ноды
...
// Параметры ниже также указывают на то, что работать вы будете с блокчейном голоса. Если убрать эти две строки
...
const cyrTag = () => {
	// Таблица транслитерации в том виде, в котором она принята на golos.io
  const _associations = {
    "а": "a",
    "б": "b",
    "в": "v",
  ...
  .get(function (err, posts) {
    ...
    for (let post of posts) {
      g.push({
        ...
        permlink: post.slug, // Если на вашем WP русские permlink, воспользуйтесь транслитерацией: cyrTag().transform(post.slug,"-")
        status: post.status,
    ...
    let posting = () => {
      ...
      switch (g[n].author) {
			...
			// Рекомендуется вместо прописывания ключей использовать переменные process.env
      case "bukowski":
	...
	// Дефолтный автор - если ни один из вариантов выше не подходит
      default:
        author.login = ''
        author.wif = ''
      }
	...
	const permlink = g[n].permlink
      ...
      golos.api.getContent(author.login, permlink,0, function (err, result) {
        ...
        // Проверяем когда было последнее обновление поста на WP
